...
#ifdef MTK_TEE_SUPPORT
#define LTC_MUTEX_GLOBAL(x)  mutex_t x = MUTEX_INITIAL_VALUE(x);
#define LTC_MUTEX_PROTO(x)   extern mutex_t x;  /* use pointer for dynamic allocation */
#define LTC_MUTEX_TYPE(x)    mutex_t x;
#define LTC_MUTEX_INIT(x)    mutex_init(x);
#define LTC_MUTEX_LOCK(x)    mutex_acquire(x);
#define LTC_MUTEX_UNLOCK(x)  mutex_release(x);
#else
